...
export const ParkashXamir = () => {
  const currentBakery = JSON.parse(localStorage.getItem("currentBakery") || "{}")
  const { data: getUsers } = useGetUsersQuery(["DRIVER"])
  const { data: bakeryDoughs } = useBakeryDoughsQuery({ bakeryId: currentBakery?.id, status: ["DELIVERED"] })
  ...
  return (
    <div>
      ...
      {bakeryDoughs?.map((item) => (
        <div key={item._id} className="my-3 rounded-[8px] bg-white p-[10px] border-[1px] border-[#FFCC15] flex items-center justify-between text-[16px] text-[#1C2C57] font-[500]">
