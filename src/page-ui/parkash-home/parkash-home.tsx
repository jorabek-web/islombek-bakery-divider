...
export const ParkashHome = () => {
  const [getDoughs] = useLazyBakeryDoughsQuery();
  ...
  useEffect(() => {
    const checkBakeryChange = () => {
      ...
      const bakeryId = currentBakery?.id || "";
  ...
  const { data: doughs } = useBakeryDoughsQuery({
    bakeryId: currentBakery?.id,
    status: ["DIVIDED"],
  });
  const { data: getBranch } = useGetBranchQuery({ id: profile?.branch || "" });

  useEffect(() => {
    if (currentBakery?.id) {
      getDoughs({ bakeryId: currentBakery.id, status: ["DELIVERED"] })
        .unwrap()
        .then((doughs) => {
          setMenu((prev) =>
            prev.map((item) =>
              item.label === "Xamir"
                ? { ...item, amount: doughs.length, loading: false }
                : item
            )
          );
        });
    }
  }, [currentBakery?.id, getDoughs]);
  ...
  return (
    <div className="pt-[35px]">
      <div className="flex items-center text-[#FFCC15] justify-start">
        <TbMessageReport size={25} onClick={() => navigate("/shikoyatlar")} />
      </div>
      ...
      <div className="border-[1px] border-[#FFCC15] rounded-[8px] p-[13px] bg-white flex items-center justify-between mt-[15px]">
        ...
        <p className="text-[#1C2C57] text-[14px] font-semibold rounded-[3px] bg-[#D9D9D9] px-[10px] shrink-0">
          {MoneyFormatter(String(getBranch?.doughPrice)) || 0}
        </p>
        <p className="text-[#1C2C57] text-[14px] font-semibold rounded-[3px] bg-[#D9D9D9] px-[10px] shrink-0">
          {MoneyFormatter(
      ...
      <div className="pt-[35px]">
        <Select aria-label="Select User" onValueChange={handleEmployeeSelect}>
          ...
          >
            <SelectValue
              id="select-label"
              placeholder={"Ishga kelgan xodimlar"}
            />
          </SelectTrigger>
          <SelectContent>
